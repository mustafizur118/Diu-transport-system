    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-header-title">
                <h4 class="pull-left page-title">List Of Bus_Category</h4>
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">Bus_Category List</h3>
                </div>
                <div class="panel-body">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered text-center">
                        ...
                        <tbody>
                        @foreach($bus_categories as $bus_category)
                            <tr>
                                <td>{{$serial++ }}</td>
                                <td>{{$bus_category->name}}</td>
                                <td>{{$bus_category->description}}</td>
                                <td>{{ucfirst($bus_category->status)}}</td>
                                <td class="d-flex">
                                    <a class="btn btn-info d-inline-block" href="{{ route('bus_category.edit', $bus_category->id) }}">Edit</a>
                                    <form class="d-inline-block pull-right" method="post" action="{{ route('bus_category.destroy', $bus_category->id) }}">
                                        @csrf
                                        @method('delete')
